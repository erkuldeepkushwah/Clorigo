  ...
  return (
    <>
      ...
      >
        ...
        {/* Mobile Menu Overlay */}
        <div 
          className={`fixed inset-0 bg-white z-50 transform transition-transform duration-300 lg:hidden flex flex-col ${
          ...
          <div className="flex flex-col gap-8 pt-10 px-6">
            {navLinks.map((link) => (
              <a 
                ...
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href, link.name)}
                className={`text-lg font-bold uppercase tracking-wider transition-colors duration-200 ${
                  activeSection === link.name ? 'text-primary' : 'text-dark hover:text-primary'
                }`}
              >
                {link.name}
              </a>
